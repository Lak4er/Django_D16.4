...
from NewsPaper.NewsPaper.settings import *
...
def my_job():
    for category in Category.subscribers_categoty.all():
        ...
        html_content = render_to_string('email_week.html', {'posts': Post.categories.filter(categories=category).filter(time_in_post__lt=datetime.now()-timedelta(days=7))})
